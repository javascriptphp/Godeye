import React, {useContext, useEffect, useState} from 'react';
import {Button, Col, message, Modal, Row, Space} from 'antd';
import {InfoCircleOutlined} from '@ant-design/icons';
import {WalletProviderContext} from "@/components/wallet/WalletProvider";
import {loginWithWallet} from "@/service";
import {SupportWallet} from "@/types";
import {ModalContext} from "antd/es/modal/context";

const ConnectWalletModal = ({isShow}: { isShow: boolean }) => {
	const useWalletProvider = () => useContext(WalletProviderContext);
	const {wallets, connectWallet} = useWalletProvider();
	const {selectedWallet, selectedAccount, disconnectWallet} = useWalletProvider();
	const [showModal, setShowModal] = useState<boolean>(isShow);
	const [messageApi, contextHolder] = message.useMessage();

	useEffect(() => {
		setShowModal(isShow);
		console.log("isShow:"+isShow)
	}, [isShow]);
	
	useEffect(() => {
		const walletName = selectedWallet?.info.name as string;
		if (selectedAccount && selectedWallet) {
			loginWithWallet(
				SupportWallet.fromName(walletName),
				selectedAccount,
				messageApi,
			).then(r => {
				console.log(r)
				setShowModal(false)
				//todo 登录成功的回调
			}).catch(e => {
				console.error(e);
			})
		}
	}, [selectedWallet, selectedAccount])
	return (
		<>
			<Modal
				title={<Row><Col span={1}><InfoCircleOutlined/></Col><Col span={22} style={{textAlign: 'center', fontSize: 20}}>Connect
					Wallet</Col></Row>}
				open={showModal}
				footer={null}
				onCancel={() => setShowModal(false)}
				maskClosable={true}
				width={450}
				style={{top: 300}}
			>
				<div>{contextHolder}</div>
				<div style={{padding: '12px', backgroundColor: '#1e1e1e', borderRadius: '8px'}}>
					<Space direction="vertical" size={20} style={{display: 'flex'}}>
						{
							Object.keys(wallets).length <= 0 ? "No Detected Wallet" :
								Object.values(wallets).map((provider: EIP6963ProviderDetail) => {
									return (<Row key={provider.info.uuid} style={{marginBottom: '10px', alignItems: 'center'}}>
											<Col span={2}>
												<img src={provider.info.icon} alt={provider.info.name}/>
											</Col>
											<Col span={1}></Col>
											<Col span={14} style={{color: 'white', fontSize: '16px', fontWeight: 'bold'}}>
												<div>{provider.info.name}</div>
											</Col>
											<Col span={6}>
												<Button type="primary" onClick={() => connectWallet(provider.info.rdns)}>
													INSTALLED
												</Button>
											</Col>
										</Row>
									)
								})
						}
					</Space>
				</div>
			</Modal>
		</>
	);
};

export default ConnectWalletModal;