...
export const getThreeMonthData = async function (symbol: string, metric: string, messageApi?: MessageInstance): Promise<ThreeMonthBuyData | ThreeMonthSellData | ErrorTypeEnum> {
	let data;
...
export const getHistoricalData = async function (symbol: string, metric: string, messageApi?: MessageInstance): Promise<HistoricalBuyData | HistoricalSellData | ErrorTypeEnum> {
	let data;
...
export const getRealtimeDataUrl = async function (metric: string, symbol: string, messageApi?: MessageInstance): Promise<string | null> {
	let url = null;
...
export const getVerificationCode = async function (email: string, messageApi: MessageInstance): Promise<boolean> {
...
export const invokeRegister = async function (registerInfo: RegisterInfo, messageApi?: MessageInstance): Promise<RegisterData | null> {
	let data = null;
...
export const invokeLogin = async function (loginInfo: LoginInfo, loginHandler: UserContextHandler, messageApi?: MessageInstance): Promise<LoginData | null> {
	...
	if (response.status === 200) {
		if (responseData.code === 200) {
			...
			const expireAt = expireCookie ? new Date(parseInt(expireCookie.split('=')[1])) : new Date(new Date().setHours(new Date().getHours()+24));
...
export const invokeLogout = async function (email: string, logoutHandler: VoidFunction, messageApi?: MessageInstance): Promise<boolean> {
	let data = false;
