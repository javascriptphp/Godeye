...
const RealtimeChart = ({
    ...
    const [buyMetricData, setBuyMetricData] = useState<
        { name: string; value: number }[]
    >([]);
    const [buyPriceData, setBuyPriceData] = useState<number[]>([]);
    const [buyThreshold, setBuyThreshold] = useState<number>(0);
    const [timestamps, setTimestamps] = useState<string[]>([]);
    const [sellMetricData, setSellMetricData] = useState<
        { name: string; value: number }[]
    >([]);
    const [sellPriceData, setSellPriceData] = useState<
        { name: string; value: number }[]
    >([]);
    ...
    const [websocketUrl, setWebsocketUrl] = useState<string>(
        "wss://stream.binance.com:9443/ws/btcusdt@kline_1s@+08:00"
    );
